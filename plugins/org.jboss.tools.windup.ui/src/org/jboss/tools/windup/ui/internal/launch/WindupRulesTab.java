...
public class WindupRulesTab extends AbstractLaunchConfigurationTab {
	...
	private void createCustomRulesGroup(Composite parent) {
		...
		listViewer = new TableViewer(group, SWT.MULTI|SWT.BORDER|SWT.FULL_SELECTION|SWT.H_SCROLL|SWT.V_SCROLL);
		GridDataFactory.fillDefaults().grab(true, true).applyTo(listViewer.getTable());
	...
	private void reloadCustomRules() {
		if (listViewer != null) {
			listViewer.setInput(Lists.newArrayList(configuration.getUserRulesDirectories()));
		}
	}
