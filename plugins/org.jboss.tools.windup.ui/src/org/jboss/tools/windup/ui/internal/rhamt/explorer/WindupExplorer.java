...
import org.jboss.tools.windup.ui.internal.explorer.CButton;
import org.jboss.tools.windup.ui.internal.explorer.IssueExplorerComparator;
...
public class WindupExplorer extends CommonNavigator {
	
	public static final String VIEW_ID = "org.jboss.tools.windup.ui.rhamt.explorer"; //$NON-NLS-1$
	...
	private WindupExplorerIssueContentService contentService;
	...
	private Composite issuesComposite;
	
	@Inject
	public WindupExplorer(WindupExplorerIssueContentService contentService) {
	...
	private void createForm(Composite parent) {
		...
		createServerSection(comp);
		createIssuesSection(comp);
		//createSystemRulesSection(comp);
		//createCustomRulesSection(comp);
	}
	...
	private void createServerSection(Composite parent) {
		...
		Section section = toolkit.createSection(container, Section.NO_TITLE_FOCUS_BOX|Section.SHORT_TITLE_BAR);
		section.setText("Server");
	...
	private void createIssuesSection(Composite parent) {
		Composite container = toolkit.createComposite(parent);
		GridLayoutFactory.fillDefaults().margins(0, 0).applyTo(container);
		GridDataFactory.fillDefaults().grab(true, true).applyTo(container);
		
		Section section = toolkit.createSection(container, Section.SHORT_TITLE_BAR);
		section.setText("Issues");
		GridDataFactory.fillDefaults().grab(true, true).applyTo(section);
		
		issuesComposite = toolkit.createComposite(section);
		toolkit.paintBordersFor(issuesComposite);
		GridLayoutFactory.fillDefaults().numColumns(2).equalWidth(false).applyTo(issuesComposite);
		GridDataFactory.fillDefaults().grab(true, true).applyTo(issuesComposite);
		section.setClient(issuesComposite);
	}
	
	private void createSystemRulesSection(Composite parent) {
		Composite container = toolkit.createComposite(parent);
		GridLayoutFactory.fillDefaults().margins(0, 10).applyTo(container);
		GridDataFactory.fillDefaults().grab(true, true).applyTo(container);
		
		Section section = toolkit.createSection(container, Section.TITLE_BAR|Section.TWISTIE);
		section.setText("System Rules");
		GridDataFactory.fillDefaults().grab(true, true).applyTo(section);
	...
	private void createCustomRulesSection(Composite parent) {
		Composite container = toolkit.createComposite(parent);
		GridLayoutFactory.fillDefaults().margins(0, 10).applyTo(container);
		GridDataFactory.fillDefaults().grab(true, true).applyTo(container);
		
		Section section = toolkit.createSection(container, Section.TITLE_BAR|Section.TWISTIE);
		section.setText("Custom Rules");
		GridDataFactory.fillDefaults().grab(true, true).applyTo(section);
	...
	public void createPartControl(Composite aParent) {
		...
		super.createPartControl(issuesComposite);
		actionGroup.dispose();
